...
- **集合相等的两套写法**：标量 `==` / `!=` 走关系产生式 (50)/(51)（操作数为
  `aexpr`）；集合 `==` / `!=` 走附加产生式 (60)/(61)（操作数为集合 `ID`），由
  操作数非终结符不同而在文法上分离，语义阶段再据类型确认。
...
终结符以双引号括起；`ID`、`NUM` 为词法单元。新增的**集合相等**(60/61)与**集
合推导式**(62/63)以 `(BONUS)` 标注。文法按非终结符分层（算术 / 逻辑 / 集合）
以保证 LALR(1) 无冲突。
...
集合测试 (58)–(61) 归约为 `bfactor`（产生式 44/45 路径），因此 `in` /
`isempty` / 集合相等都可作布尔因子参与 `&& || !` 组合。
